...
class Wave_Dataset(Dataset):
    def __init__(self, mode):
        ...
        if mode == 'train':
         ...
         # set fixed length to all samples
        self.fixed_length = 48000
    ...
    def __getitem__(self, idx):
        # read audio file
        inputs = addr2wav(self.noisy_dirs[idx])  # noisy
        targets = addr2wav(self.clean_dirs[idx])  # clean

        # transport file to tensor
        inputs = torch.from_numpy(inputs)
        targets = torch.from_numpy(targets)

        # uniform length
        if inputs.shape[0] > self.fixed_length:
            inputs = inputs[:self.fixed_length]
            targets = targets[:self.fixed_length]
        else:
            pad_len = self.fixed_length - inputs.shape[0]
            inputs = F.pad(inputs, (0, pad_len))
            targets = F.pad(targets, (0, pad_len))

        # set values between 1 and -1
