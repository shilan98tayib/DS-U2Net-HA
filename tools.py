...
########################### adaptive_weight_init  ############################################

def adaptive_weight_init(m):
    if isinstance(m, nn.Conv2d):
        ...
        # He initialization TO upsampling conv
        nn.init.kaiming_normal_(m.weight, nonlinearity='relu')
        if m.bias is not None:
            nn.init.constant_(m.bias, 0)
...
def cal_composite(dirty_wavs, clean_wavs):
    avg_csig = 0
    avg_cbak = 0
    avg_covl = 0
    ...
    avg_csig /= len(dirty_wavs)
    avg_cbak /= len(dirty_wavs)
    avg_covl /= len(dirty_wavs)

    return avg_csig, avg_cbak, avg_covl
...
def cal_snr(dirty_wavs, clean_wavs):
    avg_snr_score = 0
    for i in range(len(dirty_wavs)):
        min_length = min(clean_wavs[i].shape[0], dirty_wavs[i].shape[0])
        clean_wavs[i] = clean_wavs[i][:min_length]
        dirty_wavs[i] = dirty_wavs[i][:min_length]
...
class Writer(SummaryWriter):
    ...
    def log_scores(self, pesq, stoi, csig, cbak, covl, step):
        self.add_scalar('Metrics/PESQ', pesq, step)
        self.add_scalar('Metrics/STOI', stoi, step)
        self.add_scalar('Metrics/CSIG', csig, step)
        self.add_scalar('Metrics/CBAK', cbak, step)
        self.add_scalar('Metrics/COVL', covl, step)
