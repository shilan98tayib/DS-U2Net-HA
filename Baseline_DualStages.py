...
# Variable-Level Feature Extraction (VLFE) - d6 (for decoder part, without TLS)
...
# Variable-Level Feature Extraction (VLFE) - d5 (for decoder part, without TLS)
...
# Variable-Level Feature Extraction (VLFE) - d4 (for decoder part, without TLS)
...
# Variable-Level Feature Extraction (VLFE) - d3 (for decoder part, without TLS)
...
# Variable-Level Feature Extraction (VLFE) - d2 (for decoder part, without TLS)
...
# Variable-Level Feature Extraction (VLFE) - d1 (for decoder part, without TLS)
...
# Baseline (U2Net) + dual stages
class Baseline_DualStages(nn.Module):

    def __init__(self, in_ch=1, mid_ch=32, out_ch=64):
        super(Baseline_DualStages, self).__init__()
        ...
        # ===================== BOTTLENECK =====================
        self.ddense_1 = DilatedDenseBlock(out_ch, out_ch, 6)
        ...
        # ===================== BOTTLENECK =====================
        self.ddense_2 = DilatedDenseBlock(out_ch, out_ch, 6)
    ...
    def forward(self, x):
        ...
        # Estimate spectral mask and apply it to the noisy magnitude
        ...
        # Restore DC frequency bin
        out = F.pad(out, [0, 0, 1, 0])

        # Reconstruct waveform using the original noisy phase
