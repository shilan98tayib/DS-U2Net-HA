...
# Proposed Model (DS-U²Net-HA)
class DS_U2Net_HA(nn.Module):
    def __init__(self, in_ch=1, mid_ch=32, out_ch=32):
        super(DS_U2Net_HA, self).__init__()
        ...
        self.proposed_model = DS_U2Net_HA_model(in_ch, mid_ch, out_ch)

    def forward(self, x):
        out1 = self.proposed_model(x)
        return out1
...
class DS_U2Net_HA_model(nn.Module):

    def __init__(self, in_ch=1, mid_ch=32, out_ch=64):
        super(DS_U2Net_HA_model, self).__init__()
        self.fft_len = FFT_LEN

        # ===================== ===================== ===================== =====================
        #         =====================  First Stage of proposed model =====================
        # ===================== ===================== ===================== =====================

        self.input_layer_1 = INCONV(in_ch, out_ch)

        # ===================== ENCODER =====================
        self.en1_1 = VLFEe6(out_ch, mid_ch, out_ch)
        self.down_sampling1_1 = down_sampling(out_ch)

        self.en2_1 = VLFEe5(out_ch, mid_ch, out_ch)
        self.down_sampling2_1 = down_sampling(out_ch)

        self.en3_1 = VLFEe4(out_ch, mid_ch, out_ch)
        self.down_sampling3_1 = down_sampling(out_ch)

        self.en4_1 = VLFEe3(out_ch, mid_ch, out_ch)
        self.down_sampling4_1 = down_sampling(out_ch)

        self.en5_1 = VLFEe2(out_ch, mid_ch, out_ch)
        self.down_sampling5_1 = down_sampling(out_ch)

        self.en6_1 = VLFEe1(out_ch, mid_ch, out_ch)
        self.down_sampling6_1 = down_sampling(out_ch)

        # ===================== TSAIC BOTTLENECK =====================
        self.tsaic_1 = TSAIC(in_channels=out_ch, hidden_dim=32, alpha=1.0, beta=1.0)

        # ===================== DECODER =====================

        self.upsampling1_1 = upsampling(out_ch * 2)
        self.de1_1 = VLFEd1(out_ch * 2, mid_ch, out_ch)

        self.upsampling2_1 = upsampling(out_ch * 2)
        self.de2_1 = VLFEd2(out_ch * 2, mid_ch, out_ch)

        self.upsampling3_1 = upsampling(out_ch * 2)
        self.de3_1 = VLFEd3(out_ch * 2, mid_ch, out_ch)

        self.upsampling4_1 = upsampling(out_ch * 2)
        self.de4_1 = VLFEd4(out_ch * 2, mid_ch, out_ch)

        self.upsampling5_1 = upsampling(out_ch * 2)
        self.de5_1 = VLFEd5(out_ch * 2, mid_ch, out_ch)

        self.upsampling6_1 = upsampling(out_ch * 2)
        self.de6_1 = VLFEd6(out_ch * 2, mid_ch, out_ch)

        # output layer
        self.output_layer_1 = nn.Conv2d(out_ch, in_ch, kernel_size=1)
        self.mask_sigmoid = nn.Sigmoid()


        # ===================== ===================== ===================== =====================
        #         =====================  Second Stage of proposed model =====================
        # ===================== ===================== ===================== =====================

        self.input_layer_2 = INCONV(in_ch, out_ch)

        # ===================== ENCODER =====================
        self.en1_2 = VLFEe6(out_ch, mid_ch, out_ch)
        self.down_sampling1_2 = down_sampling(out_ch)

        self.en2_2 = VLFEe5(out_ch, mid_ch, out_ch)
        self.down_sampling2_2 = down_sampling(out_ch)

        self.en3_2 = VLFEe4(out_ch, mid_ch, out_ch)
        self.down_sampling3_2 = down_sampling(out_ch)

        self.en4_2 = VLFEe3(out_ch, mid_ch, out_ch)
        self.down_sampling4_2 = down_sampling(out_ch)

        self.en5_2 = VLFEe2(out_ch, mid_ch, out_ch)
        self.down_sampling5_2 = down_sampling(out_ch)

        self.en6_2 = VLFEe1(out_ch, mid_ch, out_ch)
        self.down_sampling6_2 = down_sampling(out_ch)

        # ===================== TSAIC BOTTLENECK =====================
        self.tsaic_2 = TSAIC(in_channels=out_ch, hidden_dim=32, alpha=1.0, beta=1.0)

        # ===================== DECODER =====================

        self.upsampling1_2 = upsampling(out_ch * 2)
        self.de1_2 = VLFEd1(out_ch * 2, mid_ch, out_ch)

        self.upsampling2_2 = upsampling(out_ch * 2)
        self.de2_2 = VLFEd2(out_ch * 2, mid_ch, out_ch)

        self.upsampling3_2 = upsampling(out_ch * 2)
        self.de3_2 = VLFEd3(out_ch * 2, mid_ch, out_ch)

        self.upsampling4_2 = upsampling(out_ch * 2)
        self.de4_2 = VLFEd4(out_ch * 2, mid_ch, out_ch)

        self.upsampling5_2 = upsampling(out_ch * 2)
        self.de5_2 = VLFEd5(out_ch * 2, mid_ch, out_ch)

        self.upsampling6_2 = upsampling(out_ch * 2)
        self.de6_2 = VLFEd6(out_ch * 2, mid_ch, out_ch)

        # output layer
        self.output_layer_2 = nn.Conv2d(out_ch, in_ch, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # input layer
        hx = self.input_layer_1(hx02)  # [B, out_ch, F, T]

        # ===================== ENCODER PATH =====================
        # encoder stage 1
        hx1, hx1_1, hx1_2, hx1_3, hx1_4, hx1_5, hx1_6 = self.en1_1(hx)
        hx1 = self.down_sampling1_1(hx1)

        # encoder stage 2
        hx2, hx2_1, hx2_2, hx2_3, hx2_4, hx2_5 = self.en2_1(hx1)
        hx2 = self.down_sampling2_1(hx2)

        # encoder stage 3
        hx3, hx3_1, hx3_2, hx3_3, hx3_4 = self.en3_1(hx2)
        hx3 = self.down_sampling3_1(hx3)

        # encoder stage 4
        hx4, hx4_1, hx4_2, hx4_3 = self.en4_1(hx3)
        hx4 = self.down_sampling4_1(hx4)

        # encoder stage 5
        hx5, hx5_1, hx5_2 = self.en5_1(hx4)
        hx5 = self.down_sampling5_1(hx5)

        # encoder stage 6
        hx6, hx6_1 = self.en6_1(hx5)
        hx6 = self.down_sampling6_1(hx6)

        # ===================== TSAIC BOTTLENECK =====================
        out = self.tsaic_1(hx6)

        # ===================== DECODER PATH =====================

        # decoder stage 1 (Compatible with stage 6)
        out = self.upsampling1_1(torch.cat([out, hx6], dim=1))
        out = self.de1_1(out, hx6_1)

        # decoder stage 2 (Compatible with stage 5)
        out = self.upsampling2_1(torch.cat([out, hx5], dim=1))
        out = self.de2_1(out, hx5_1, hx5_2)

        # decoder stage 3 (Compatible with stage 4)
        out = self.upsampling3_1(torch.cat([out, hx4], dim=1))
        out = self.de3_1(out, hx4_1, hx4_2, hx4_3)

        # decoder stage 4 (Compatible with stage 3)
        out = self.upsampling4_1(torch.cat([out, hx3], dim=1))
        out = self.de4_1(out, hx3_1, hx3_2, hx3_3, hx3_4)

        # decoder stage 5 (Compatible with stage 2)
        out = self.upsampling5_1(torch.cat([out, hx2], dim=1))
        out = self.de5_1(out, hx2_1, hx2_2, hx2_3, hx2_4, hx2_5)

        # decoder stage 6 (Compatible with stage 1)
        out = self.upsampling6_1(torch.cat([out, hx1], dim=1))
        out = self.de6_1(out, hx1_1, hx1_2, hx1_3, hx1_4, hx1_5, hx1_6)
        ...
        # input layer
        hx = self.input_layer_2(out)  # [B, out_ch, F, T]

        # ===================== ENCODER PATH =====================
        # encoder stage 1
        hx1, hx1_1, hx1_2, hx1_3, hx1_4, hx1_5, hx1_6 = self.en1_2(hx)
        hx1 = self.down_sampling1_2(hx1)

        # encoder stage 2
        hx2, hx2_1, hx2_2, hx2_3, hx2_4, hx2_5 = self.en2_2(hx1)
        hx2 = self.down_sampling2_2(hx2)

        # encoder stage 3
        hx3, hx3_1, hx3_2, hx3_3, hx3_4 = self.en3_2(hx2)
        hx3 = self.down_sampling3_2(hx3)

        # encoder stage 4
        hx4, hx4_1, hx4_2, hx4_3 = self.en4_2(hx3)
        hx4 = self.down_sampling4_2(hx4)

        # encoder stage 5
        hx5, hx5_1, hx5_2 = self.en5_2(hx4)
        hx5 = self.down_sampling5_2(hx5)

        # encoder stage 6
        hx6, hx6_1 = self.en6_2(hx5)
        hx6 = self.down_sampling6_2(hx6)

        # ===================== TSAIC BOTTLENECK =====================
        out = self.tsaic_2(hx6)

        # ===================== DECODER PATH =====================

        # decoder stage 1 (Compatible with stage 6)
        out = self.upsampling1_2(torch.cat([out, hx6], dim=1))
        out = self.de1_2(out, hx6_1)

        # decoder stage 2 (Compatible with stage 5)
        out = self.upsampling2_2(torch.cat([out, hx5], dim=1))
        out = self.de2_2(out, hx5_1, hx5_2)

        # decoder stage 3 (Compatible with stage 4)
        out = self.upsampling3_2(torch.cat([out, hx4], dim=1))
        out = self.de3_2(out, hx4_1, hx4_2, hx4_3)

        # decoder stage 4 (Compatible with stage 3)
        out = self.upsampling4_2(torch.cat([out, hx3], dim=1))
        out = self.de4_2(out, hx3_1, hx3_2, hx3_3, hx3_4)

        # decoder stage 5 (Compatible with stage 2)
        out = self.upsampling5_2(torch.cat([out, hx2], dim=1))
        out = self.de5_2(out, hx2_1, hx2_2, hx2_3, hx2_4, hx2_5)

        # decoder stage 6 (Compatible with stage 1)
        out = self.upsampling6_2(torch.cat([out, hx1], dim=1))
        out = self.de6_2(out, hx1_1, hx1_2, hx1_3, hx1_4, hx1_5, hx1_6)

        # Direct estimation of the clean speech magnitude
        out = self.output_layer_2(out)

        # Ensure the estimated magnitude is non-negative,
        # since magnitude spectra cannot contain negative values.
        out = torch.relu(out)
